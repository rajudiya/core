...
{
    const EXTENSIONS_PATH = 'magentobrain_extensions';
    const URL_EXTENSIONS  = 'http://www.magentobrain.com/shopfeed/index/extensiondata';
     ...
     * @var array
     */
    private $magentobrainExtensions = [];
     ...
     * @var array
     */
    private $moduleIgnoreList = ['MagentoBrain_Core', 'MagentoBrain_Tips'];

    /**
     * @var array
     */
    private $magentobrainPrefixList = ['MagentoBrain_', 'AdjustWare_'];

    /**
     * @var array
     */
    private $moduleDirs = ['MagentoBrain', 'AdjustWare'];
     ...
     * @return array
     */
    public function getMagentoBrainExtensions($restrictedDelete = false)
    {
        if (!$this->magentobrainExtensions) {
            $modules = $this->moduleList->getNames();

            $dispatchResult = new \Magento\Framework\DataObject($modules);
            $modules = $dispatchResult->toArray();

            foreach ($this->magentobrainPrefixList as $prefix) {
                foreach ($modules as $item) {
                    if (strpos($item, $prefix) !== false) {
                        $this->magentobrainExtensions[] = $item;
                    }
                }
            }
        }
        ...
        if ($restrictedDelete) {
            foreach ($this->moduleIgnoreList as $value) {
                if (array_search($value, $this->magentobrainExtensions) !== false) {
                    unset($this->magentobrainExtensions[array_search($value, $this->magentobrainExtensions)]);
                }
            }
        }
        sort($this->magentobrainExtensions);

        return $this->magentobrainExtensions;
    }
